...
1. 
//1 3 6 4 2 5 (все по порядку согласно очереди (синхронные, асинхронные: промисы, setTimeout))

2. 
// 1 3 2 ("a" не выводится в console.log,она передается в then, по выполнению операций порядок: 1 3 а 2)

3. 
// a b c (первый resolve передается в then (a), finally выполняется, но не повлияет на значение, .catch игнорируем, тк произошел resolve)
...
5. 
// 1 4 3 2 (все по порядку согласно очереди (синхронные, асинхронные: промисы, setTimeout))
